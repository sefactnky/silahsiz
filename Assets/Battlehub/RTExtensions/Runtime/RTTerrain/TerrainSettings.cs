...
namespace Battlehub.RTTerrain
{
    ...
    public class TerrainSettings : EditorExtension, ITerrainSettings
    {
        ...
        private const string k_defaultTextureKey = "Battlehub.RTTerrain.TerrainSettings.Texture";

        private async void SetDefaultTexture(Texture2D texture, Action callback)
        {
            var copy = Instantiate(texture);
            copy.name = texture.name;

            await m_editor.SetValueAsync(k_defaultTextureKey, copy);
            callback?.Invoke();
        }

        private async void GetDefaultTexture(Action<Texture2D> callback)
        {
            var texture = await m_editor.GetValueAsync<Texture2D>(k_defaultTextureKey);
