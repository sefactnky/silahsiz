...
namespace Battlehub.ProBuilderIntegration
{
    ...
    public class MeshState
    {
        ...
        {
            Positions = positions;
            //Faces = faces.Select(f => new PBFace(f, recordUV)).ToList(); /*always record uvs*/
            Faces = faces.Select(f => new PBFace(f, true)).ToList();
            Textures = textures;
        }
    }
    ...
    public static class IMeshEditorExt
    {
        public static MeshSelection Select(Material material)
        {
            ...
            ProBuilderMesh[] meshes = UnityObjectExt.FindObjectsByType<ProBuilderMesh>();
            foreach (ProBuilderMesh mesh in meshes)
            {
