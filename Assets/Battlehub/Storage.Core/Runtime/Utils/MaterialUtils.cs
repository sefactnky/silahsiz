...
namespace Battlehub.Storage
{
    ...
    public static class RenderPipelineInfo
    {
                ...
                switch (Type)
                {
                    ...
                    default:
                        if (m_msaaProperty != null)
                        {
                            try
                            {
                                return Convert.ToInt32(m_msaaProperty.GetValue(UnityGraphicsSettings.renderPipelineAsset));
                            }
                            catch (Exception e)
                            {
                                Debug.LogError(e);
        ...
        static RenderPipelineInfo()
        {
            ...
            {
                Type pipelineType = UnityGraphicsSettings.renderPipelineAsset.GetType();
                ...
                {
                    Debug.Log(UnityGraphicsSettings.renderPipelineAsset.GetType());
                    Type = RPType.Unknown;
                    ColorPropertyID = Shader.PropertyToID("_Color");
                    MainTexturePropertyID = Shader.PropertyToID("_MainTex");
                }
    ...
    public class UniversalLitMaterialUtils : IMaterialUtils
    {
        ...
        public void SetMaterialKeywords(Material material)
        {
            if (material == null || material.shader == null 
                || material.shader.name != RenderPipelineInfo.DefaultShaderName &&
        ...
        private static void _SetMaterialKeywords(Material material)
        {
            ...
            if (material.HasProperty("_WorkflowMode"))
            {
                ...
                if (isSpecularWorkFlow && material.HasTexture("_SpecGlossMap"))
                    hasGlossMap = material.GetTexture("_SpecGlossMap") != null;
                else if (material.HasTexture("_MetallicGlossMap"))
