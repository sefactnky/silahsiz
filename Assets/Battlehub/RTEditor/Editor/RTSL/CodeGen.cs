...
namespace Battlehub.RTSL
{
    ...
    public class CodeGen
    {
        ...
        /// <summary>
        /// Get all public instance declared only properties (excluding indexers) (including base class properties if the base class is a generic type definition)
        /// </summary>
        /// <param name="type"></param>
        /// <returns></returns>
        public static PropertyInfo[] GetAllProperties(Type type)
        {
            ...
            {
                var baseTypeProperties = GetAllDeclaredOnlyProperties(type.BaseType);
                var properties = GetAllDeclaredOnlyProperties(type);
                return properties.Union(baseTypeProperties).ToArray();
            }

            return GetAllDeclaredOnlyProperties(type);
        }

        private static PropertyInfo[] GetAllDeclaredOnlyProperties(Type type)
        {
        ...
        /// <summary>
        /// Get all public instance declared only fields (including base class fields if the base class is a generic type definition)
        /// </summary>
        /// <param name="type"></param>
        /// <returns></returns>
        public static FieldInfo[] GetFields(Type type)
        {
            if(type.BaseType != null && type.BaseType.IsGenericType)
            {
                var baseTypeFields = GetDeclaredOnlyFields(type.BaseType);
                var fields = GetDeclaredOnlyFields(type);
                return fields.Union(baseTypeFields).ToArray();
            }

            return GetDeclaredOnlyFields(type);
        }

        private static FieldInfo[] GetDeclaredOnlyFields(Type type)
        {
