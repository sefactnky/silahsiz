    ...
    {           
        ...
        public static void CreateRuntimeEditor()
        {
            ...
            if (!eventSystem)
            {
                ...
                es.name = "EventSystem";
                es.transform.SetParent(editor.transform);
            ...
            GameObject scene = GameObject.FindGameObjectWithTag(ExposeToEditor.HierarchyRootTag);
            if (scene == null)
            {
        ...
        [MenuItem("Tools/Runtime Editor/Create Asset Database", priority = 3)]
        public static void ConfigureAssetDatabase()
        {
            ExternalAssetListGen.GenerateBuiltInAssetsList();
            GameObject[] hierarchyRoots = GameObject.FindGameObjectsWithTag(ExposeToEditor.HierarchyRootTag);
            var sceneAssetsList = ExternalAssetListGen.GenerateAssetsList(hierarchyRoots);

            UnityObject prefab = AssetDatabase.LoadAssetAtPath(BHRoot.PackageRuntimeContentPath + "/RTEditor/Prefabs/Models/AssetDatabaseModel.prefab", typeof(GameObject));

            GameObject editor = GameObject.Find("RTEditor");
            GameObject assetDatabaseModel = GameObject.Find("AssetDatabaseModel");
            if (assetDatabaseModel == null)
            {
                assetDatabaseModel = (GameObject)PrefabUtility.InstantiatePrefab(prefab);
                assetDatabaseModel.AddComponent<RTSLIgnore>();
                assetDatabaseModel.name = "AssetDatabaseModel";

                if (editor != null)
                {
                    Transform models = editor.transform.Find("Models");
                    if (models != null)
                    {
                        assetDatabaseModel.transform.SetParent(models, false);
                    }
                }
            }
            Type hostType = Type.GetType(RuntimeAssetDatabase.AssetDatabaseHostTypeName);
            if (hostType == null)
            {
                return;
            }

            var host = assetDatabaseModel.GetComponentInChildren(hostType, true);
            if (host != null)
            {
                if (sceneAssetsList != null)
                {
                    const string externalAssetsPropertyName = "ExternalAssets";
                    var externalAssets = ((ExternalAssetList[])hostType.GetProperty(externalAssetsPropertyName).GetValue(host)).ToList();
                    if (externalAssets == null)
                    {
                        Debug.LogWarning($"Can't find {hostType.FullName}.{externalAssetsPropertyName} property");
                        return;
                    }

                    if (!externalAssets.Contains(sceneAssetsList))
                    {
                        externalAssets.Add(sceneAssetsList);
                    }

                    hostType.GetProperty(externalAssetsPropertyName).SetValue(host, externalAssets.Where(list => list != null).ToArray());
            ...
            {
                Debug.LogWarning("Can't find AssetDatabase Host");
            }
        ...
        [MenuItem("Tools/Runtime Editor/Add Mobile support for RTEditor", priority = 4)]
