...
namespace Battlehub.RTSL
{
    ...
    internal class ProjectAsyncSafe : IProjectAsyncSafe
    {
        ...
        public async Task SetRootPathAsync(string rootPath)
        {
            ...
            {
                await m_wrappedProject.SetRootPathAsync(rootPath);
            }
        }
                    ...
                    if (!ct.IsCancellationRequested)
                    {
                        ...
                        if (assetLib == null)
                        {
                            if (IsBuiltinLibrary(assetLibraryID))
                            {
                                ...
                                {
                                    Debug.LogWarningFormat("Asset Library was not found : {0}. Click Tools->Runtime SaveLoad (Legacy)->Update Libraries.", assetLibrary);
                                }
                            }
    ...
    public class ProjectAsyncImpl<TID> : IProjectAsync, IProjectEvents, IProjectState, IProjectUtils, IDisposable where TID : IEquatable<TID>
    {
        ...
        public bool IsStatic(UnityObject obj)
        {
            return AssetDB.IsStaticResourceID(AssetDB.ToID(obj));
        }
        ...
        {
            return m_storage.SetRootPathAsync(rootPath);
        }
        ...
        public async Task<byte[][]> GetPreviewsAsync(ProjectItem[] projectItems, CancellationToken ct)
        {
            ...
            return previews.Select(preview => preview != null ? preview.PreviewData : null).ToArray();
        }
        ...
        public async Task<object[]> GetDependenciesAsync(object obj, bool exceptMappedObjects, CancellationToken ct)
        {
            ...
            {
                Debug.LogWarningFormat(string.Format("PersistentClass for {0} does not exist. To create or edit persistent classes click Tools->Runtime SaveLoad (Legacy)->Persistent Classes->Edit.", obj.GetType()), "obj");
                return new object[0];
            }
        ...
        protected async Task<ProjectItem[]> SaveAsync(ProjectItem[] existingAssetItems, ProjectItem[] folders, byte[][] previewData, object[] objects, string[] nameOverrides, CancellationToken ct)
        {
            ...
            for (int o = 0; o < objects.Length; ++o)
            {
                ...
                if (persistentType == null)
                {
                    Debug.LogWarningFormat(string.Format("PersistentClass for {0} does not exist. Tools->Runtime SaveLoad (Legacy)->Persistent Classes->Edit", obj.GetType()), "obj");
                    continue;
                }
        ...
        public async Task<ProjectItem[]> DuplicateAsync(ProjectItem[] projectItems, CancellationToken ct)
        {
            ...
            {
                var go = loadedObjects[i] as GameObject;
                var name = loadedObjects[i].name;
                
                loadedObjects[i] = UnityObject.Instantiate(loadedObjects[i], go != null ? go.transform.parent : null);
                loadedObjects[i].name = name;
                loadedObjects[i].hideFlags = HideFlags.None;
            }
