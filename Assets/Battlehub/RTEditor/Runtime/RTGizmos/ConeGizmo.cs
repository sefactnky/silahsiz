...
namespace Battlehub.RTGizmos
{
    public abstract class ConeGizmo : BaseGizmo
    {
        ...
        {
            get { return Matrix4x4.TRS(TargetTransform.TransformPoint(Vector3.forward * Height), TargetTransform.rotation, Scale); }
        }
        ...
        protected override void OnCommandBufferRefresh(IRTECamera camera)
        {
            ...
            GizmoUtility.DrawConeHandles(camera.RTECommandBuffer, TargetTransform.TransformPoint(Vector3.forward * Height), TargetTransform.rotation, Scale, LineProperties);
            GizmoUtility.DrawWireCone(camera.RTECommandBuffer, Height, Radius, TargetTransform.position, TargetTransform.rotation, Vector3.one, LineProperties);
            if (IsDragging)
            {
                GizmoUtility.DrawSelection(camera.RTECommandBuffer, TargetTransform.TransformPoint(Vector3.forward * Height + Vector3.Scale(HandlesPositions[DragIndex], Scale)), TargetTransform.rotation, Scale, SelectionProperties);
            }
        }
    }
}
