...
namespace Battlehub.RTEditor
{
    ...
    public static class IWindowManagerExt
    {
        ...
        { 
            ...
            wm.CreateWindow(BuiltInWindowNames.Scene, out sceneWd, out sceneContent, out _);

            WindowDescriptor gameWd;
            GameObject gameContent;
            wm.CreateWindow(BuiltInWindowNames.Game, out gameWd, out gameContent, out _);

            WindowDescriptor inspectorWd;
            GameObject inspectorContent;
            wm.CreateWindow(BuiltInWindowNames.Inspector, out inspectorWd, out inspectorContent, out _);

            WindowDescriptor consoleWd;
            GameObject consoleContent;
            wm.CreateWindow(BuiltInWindowNames.Console, out consoleWd, out consoleContent, out _);

            WindowDescriptor hierarchyWd;
            GameObject hierarchyContent;
            wm.CreateWindow(BuiltInWindowNames.Hierarchy, out hierarchyWd, out hierarchyContent, out _);

            WindowDescriptor projectWd;
            GameObject projectContent;
            
            wm.CreateWindow(BuiltInWindowNames.Project, out projectWd, out projectContent, out _);
            ...
            LayoutInfo game = wm.CreateLayoutInfo(gameContent.transform, gameWd);
    ...
    public class WindowManager : MonoBehaviour, IWindowManager
    {
        ...
        private Component m_ownInputModuleComponent;
        ...
        private void Awake()
        {
            ...
            if (m_activeWorkspace == null)
            {
                if (m_dockPanels != null)
                {
                    ...
                    {
                        m_dialogManager = UnityObjectExt.FindAnyObjectByType<DialogManager>();
                    }
        ...
        private void Start()
        {
            ...
            {
                EventSystem eventSystem = m_editor.EventSystem;
                if (eventSystem != null && eventSystem.GetComponent<BaseInputModule>() == null)
                {
                    var rteStandaloneInputModule = gameObject.AddComponent<RTEStandaloneInputModule>();
                    m_ownInputModuleComponent = rteStandaloneInputModule;
                    m_inputModule = rteStandaloneInputModule;
                }
                else
                {
                    Debug.LogWarning("RTEStandaloneInputModule not found");
                }
            }
            ...
            if (ActiveWorkspace?.ToolsRoot != null)
            {
                Transform tools = CreateWindow(RuntimeWindowType.ToolsPanel.ToString().ToLower(), out var wd, out var content, out var isDialog);
                if (tools != null)
                {
                    SetTools(tools);
                }
        ...
        private void OnDestroy()
        {
            ...
            if (m_ownInputModuleComponent != null)
            {
                Destroy(m_ownInputModuleComponent);
                m_ownInputModuleComponent = null;
            }
        }
        ...
        /*
        private string UnifyWindowTypeName(string windowTypeName)
        {
            #pragma warning disable CS0618 // Type or member is obsolete
            if (windowTypeName == BuiltInWindowNames.Project)
            {
                windowTypeName = BuiltInWindowNames.AssetDatabase;
            }
            else if (windowTypeName == BuiltInWindowNames.ImportAssets)
            {
                windowTypeName = BuiltInWindowNames.AssetDatabaseImport;
            }
            else if (windowTypeName == BuiltInWindowNames.SelectAssetLibrary)
            {
                windowTypeName = BuiltInWindowNames.AssetDatabaseImportSource;
            }
            else if (windowTypeName == BuiltInWindowNames.SaveAsset)
            {
                windowTypeName = BuiltInWindowNames.AssetDatabaseSave;
            }
            else if (windowTypeName == BuiltInWindowNames.SaveScene)
            {
                windowTypeName = BuiltInWindowNames.AssetDatabaseSaveScene;
            }
            else if (windowTypeName == BuiltInWindowNames.SelectObject)
            {
                windowTypeName = BuiltInWindowNames.AssetDatabaseSelect;
            }
        ...
        */
        ...
        /*
        private string RewriteWindowTypeName(string windowTypeName)
        {
            ...
            {
                if (windowTypeName == BuiltInWindowNames.AssetDatabase)
                {
                    windowTypeName = BuiltInWindowNames.Project;
                }   
                else if (windowTypeName == BuiltInWindowNames.AssetDatabaseImport)
                {
                    windowTypeName = BuiltInWindowNames.ImportAssets;
                }
                else if (windowTypeName == BuiltInWindowNames.AssetDatabaseImportSource)
                {
                    windowTypeName = BuiltInWindowNames.SelectAssetLibrary;
                }
                else if (windowTypeName == BuiltInWindowNames.AssetDatabaseSave)
                {
                    windowTypeName = BuiltInWindowNames.SaveAsset;
                }
                else if (windowTypeName == BuiltInWindowNames.AssetDatabaseSaveScene)
                {
                    windowTypeName = BuiltInWindowNames.SaveScene;
                }
                else if (windowTypeName == BuiltInWindowNames.AssetDatabaseSelect)
                {
                    windowTypeName = BuiltInWindowNames.SelectObject;
                }
            }
            else
            {
        ...
        */
        ...
        public void OverrideWindow(string windowTypeName, WindowDescriptor descriptor)
        {
            ...
            else if (windowTypeName == BuiltInWindowNames.Scene)
            {
                m_sceneWindow = descriptor;
            }
            else if (windowTypeName == BuiltInWindowNames.Game)
            {
                m_gameWindow = descriptor;
            }
            else if (windowTypeName == BuiltInWindowNames.Hierarchy)
            {
                m_hierarchyWindow = descriptor;
            }

            else if (windowTypeName == BuiltInWindowNames.Inspector)
            {
                m_inspectorWindow = descriptor;
            }
            else if (windowTypeName == BuiltInWindowNames.Project)
            {
                m_projectWindow = descriptor;
            }
            else if (windowTypeName == BuiltInWindowNames.Console)
            {
                m_consoleWindow = descriptor;
            }
            else if (windowTypeName == BuiltInWindowNames.Animation)
            {
            ...
            else if (windowTypeName == BuiltInWindowNames.OpenProject)
            {
                m_openProjectDialog = descriptor;
            }
            else if (windowTypeName == BuiltInWindowNames.ToolsPanel)
            {
            ...
            else if (windowTypeName == BuiltInWindowNames.About)
            {
            ...
            else if (windowTypeName == BuiltInWindowNames.SelectColor)
            {
                m_selectColorDialog = descriptor;
            }
            else if (windowTypeName == BuiltInWindowNames.SelectAnimationProperties)
            {
                m_selectAnimationPropertiesDialog = descriptor;
            }
            else if (windowTypeName == BuiltInWindowNames.SaveFile)
            {
                m_saveFileDialog = descriptor;
            }
            else if (windowTypeName == BuiltInWindowNames.OpenFile)
            {
                m_openFileDialog = descriptor;
            }
            else if(windowTypeName == BuiltInWindowNames.Empty)
            {
                m_empty = descriptor;
            }
            else if(windowTypeName == BuiltInWindowNames.EmptyDialog)
            {
        ...
        {
            string windowTypeName = BuiltInWindowNames.ToolsPanel;
        ...
        public WindowDescriptor GetWindowDescriptor(string windowTypeName, out bool isDialog)
        {
            ...
            {
                if (windowTypeName == BuiltInWindowNames.Scene)
                {
                    wd = m_sceneWindow;
                }
                else if (windowTypeName == BuiltInWindowNames.Game)
