...
namespace Battlehub.RTEditor.Models
{
    public abstract class BaseLayoutStorageModel : MonoBehaviour, ILayoutStorageModel
    {
        ...
        private void ToLayout(PersistentLayoutInfo persistentLayoutInfo, LayoutInfo layoutInfo, GameObject tabPrefab)
        {
            ...
            if (!string.IsNullOrEmpty(persistentLayoutInfo.WindowType))
            {
                ...
                string windowType = persistentLayoutInfo.WindowType;
                if (windowType == RuntimeWindowType.Project.ToString().ToLower() || windowType == BuiltInWindowNames.AssetDatabase)
                {
                    var editor = IOC.Resolve<IRuntimeEditor>();

                    if (editor != null && editor.CompatibilityMode == CompatibilityMode.LegacyRTSL)
                    {
                        windowType = RuntimeWindowType.Project.ToString().ToLower();
                    }
                    else
                    {
                        windowType = BuiltInWindowNames.AssetDatabase;
                    }
                }
                
                wm.CreateWindow(windowType, out wd, out content, out _);
    ...
    public class PlayerPrefsLayoutStorageModel : BaseLayoutStorageModel
    {
        ...
        {
            return $"{m_keyPrefix}Battlehub.RTEditor.Layout{path}";
        }

        public override bool LayoutExists(string path)
        {
            return PlayerPrefs.HasKey(ToKey(path));
        }
        ...
        {
            string serializedLayout = PlayerPrefs.GetString(ToKey(path));
        ...
        public override void SaveLayout(string path, LayoutInfo layout)
        {
            ...
            PlayerPrefs.SetString(key, serializedLayout);
            PlayerPrefs.Save();
