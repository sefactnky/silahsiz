    ...
    public class GroupingModel : MonoBehaviour, IGroupingModel
    {
        ...
        public virtual bool CanGroup(GameObject[] gameObjects)
        {
            ...
            if (editor != null)
            {
                ...
                {
                    if (!editor.IsPrefabOperationAllowed(gameObjects[i]) || editor.CurrentPrefab == gameObjects[i])
                    {
                        return false;
                    }
                }
        ...
        public virtual bool CanMerge(GameObject[] gameObjects)
        {
            ...
            if (editor != null)
            {
                for (int i = 0; i < gameObjects.Length; i++)
                {
                    if (!editor.IsPrefabOperationAllowed(gameObjects[i]))
                    {
                        return false;
                    }
                }
        ...
        protected virtual GameObject Group(GameObject[] gameObjects, string groupName, Transform center, Transform parent)
        {
            ...
            IRTE editor = IOC.Resolve<IRTE>();
            if (editor != null && editor.HierarchyRoot != null)
            {
                group.transform.SetParent(editor.HierarchyRoot.transform);
            }
        ...
        protected virtual bool CanUngroup(GameObject gameObject)
        {
            ...
            if (editor != null)
            {
                if (editor.IsInstanceRoot(gameObject) || !editor.IsPrefabOperationAllowed(gameObject))
                {
                    return false;
                }
            }
