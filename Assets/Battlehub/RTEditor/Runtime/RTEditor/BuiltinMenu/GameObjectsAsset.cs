...
namespace Battlehub.RTEditor
{
    ...
    public class GameObjectsAsset : ScriptableObject
    {
        ...
        public virtual GameObject Instantiate(string path)
        {
            ...
            {
                Canvas canvas = UnityObjectExt.FindObjectsByType<Canvas>().Where(c => (c.GetComponentInParent<RTSLIgnore>() == null) && c.hideFlags == HideFlags.None).FirstOrDefault();
                if (canvas != null && !prefab.GetComponent<Canvas>())
                {
                    go = Instantiate(prefab, canvas.transform);
                }
                else
                {
                    go = prefab != null ? Instantiate(prefab, rte.InstanceRoot?.transform) : instantiateFunc(rte.InstanceRoot?.transform);
                }
            }
            else
            {
