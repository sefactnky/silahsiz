...
namespace Battlehub.RTEditor
{
    ...
    public class ObjectEditor : PropertyEditor<UnityObject>, IPointerEnterHandler, IPointerExitHandler
    {
        ...
        private void OnSelect()
        {
            ...
            string selectObjectWindowName = memberInfoType.Name == "Object" ? BuiltInWindowNames.SelectObject : "Select" + memberInfoType.Name;

            if (!wm.IsWindowRegistered(selectObjectWindowName))
            {
                selectObjectWindowName = BuiltInWindowNames.SelectObject;
            }

            IOC.Resolve<IWindowManager>().CreateDialogWindow(selectObjectWindowName, select + memberInfoTypeName,
                (sender, args) =>
                {
        ...
        {
            if (CurrentValue != null && !HasMixedValues())
            {
                var editor = IOC.Resolve<IRuntimeEditor>();
                if (editor == null)
                {
                    Editor.Selection.activeObject = CurrentValue;
                    return;
                }

                if (CurrentValue is GameObject || CurrentValue is Component)
                {
                    var id = editor.GetSubAssetID(CurrentValue);
                    if (id != ID.Empty)
                    {
                ...
                {
                    if (editor.IsExternalAsset(CurrentValue))
                    {
                        ...
                        var header = lc.GetString("ID_RTEditor_PE_ObjectEditor_CloneExternalAsset", "Clone External Asset");
                        var text = string.Format(lc.GetString("ID_RTEditor_PE_ObjectEditor_CloneForEditing", "Clone '{0}' for editing?"), name);
                        wm.Confirmation(header, text, (dialog, args) =>
                        {
                        ...
                        (dialog, cancelArgs) => { },
                        lc.GetString("ID_RTEditor_PE_ObjectEditor_Clone", "Clone"),
                        lc.GetString("ID_RTEditor_PE_ObjectEditor_Cancel", "Cancel"));
                    }
                    else
                    {
                        Editor.Selection.activeObject = CurrentValue;
