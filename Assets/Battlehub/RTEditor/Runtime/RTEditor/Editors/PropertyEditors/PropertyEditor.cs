...
namespace Battlehub.RTEditor
{
    ...
    public class PropertyEditor : MonoBehaviour
    {
        ...
        public void Init(object[] target, MemberInfo memberInfo, PropertyDescriptor[] childDescriptors, string label, bool enableUndo = true,
            PropertyEditorCallback beginRecordCallback = null,
            PropertyEditorCallback endRecordCallback = null,
            PropertyEditorCallback afterRedoCallback = null,
            PropertyEditorCallback afterUndoCallback = null,
            ...
            Action<object, object> eraseTargetCallback = null)
        {
            Init(target, target, memberInfo, eraseTargetCallback, label, valueChangingCallback, valueChangedCallback, endEditCallback, enableUndo, childDescriptors, beginRecordCallback, endRecordCallback, afterRedoCallback, afterUndoCallback, reloadCallback);
        }
        ...
        public void Reload(bool force = false, bool raiseValueChanged = false)
        {
            if(m_isEditing)
            {
                return;
...
#pragma warning restore CS0618 // Type or member is obsolete
            ReloadOverride(force, raiseValueChanged);
            m_lockValue = false;
        }

        [Obsolete("User ReloadOverride(bool force, bool raiseValueChanged)")]
        protected virtual void ReloadOverride(bool force)
        {

        }

        protected virtual void ReloadOverride(bool force, bool raiseValueChanged)
        {
        ...
        private void NotifyBeginEdit()
        {
            ...
            if (inspectorModel != null)
            {
                ...
                {
                    NotifyBeginEdit(inspectorModel, componentEditor);
                }
            }
        }

        internal virtual void NotifyBeginEdit(IInspectorModel inspectorModel, ComponentEditor componentEditor)
        {
        ...
        private void NotifyEndEdit()
        {
            ...
            if (inspectorModel != null)
            {
                ...
                if (componentEditor != null)
                {
                    NotifyEndEdit(inspectorModel, componentEditor);
                }
            }
        }

        internal virtual void NotifyEndEdit(IInspectorModel inspectorModel, ComponentEditor componentEditor)
        {
...
#pragma warning restore CS0612
        ...
        public override bool IsAutoReloadEnabled
        {
            ...
            set
            {
                if (value)
                {
                    m_nextReload = Time.unscaledTime;
                }
                else
                {
        ...
        public T GetValue(int index = -1)
        {
            object value = index == -1 ?
                _GetValue(Target, Accessor) :
                _GetValue(Targets[index], Accessors[index]);
        ...
        {
            object value = _GetValue(target, accessor);
            return value != null ? (T)value : default;
        }
        ...
        private T[] m_initalValues;
        internal override void NotifyBeginEdit(IInspectorModel inspectorModel, ComponentEditor componentEditor)
        {
            m_initalValues = GetValues().ToArray();

            base.NotifyBeginEdit(inspectorModel, componentEditor);
        }

        internal override void NotifyEndEdit(IInspectorModel inspectorModel, ComponentEditor componentEditor)
        {
            var values = GetValues().ToArray();
            List<object> dirty = null; 
            for (int i = 0; i < m_initalValues.Length; ++i)
            {
                if (!Equals(m_initalValues[i], values[i]))
                {
                    if (dirty == null)
                    {
                        dirty = new List<object>();
                    }

                    dirty.Add(componentEditor.Components[i]);   
                }
            }

            if (dirty != null)
            {
                inspectorModel.SetDirty(dirty.ToArray());
            }

            base.NotifyEndEdit(inspectorModel, componentEditor);
        }
        ...
        {
            if (m_nextReload > Time.unscaledTime)
            {
                return;
            }

            m_nextReload = Time.unscaledTime + m_reloadInterval;
        ...
        {
            base.ReloadOverride(force, raiseValueChanged);

            T value = GetValue();
            bool valueChanged = !Equals(CurrentValue, value);
            if (force || valueChanged) 
            {
                CurrentValue = value;
                SetInputField(value);

                if (raiseValueChanged && valueChanged)
                {
                    RaiseValueChanged();
                }
