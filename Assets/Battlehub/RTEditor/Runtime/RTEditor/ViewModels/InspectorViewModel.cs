...
namespace Battlehub.RTEditor.ViewModels
{
    ...
    public class InspectorViewModel : ViewModel
    {
        ...
        public virtual async void OnSave()
        {
            ...
            {
                await SaveObjects(SelectedObjects);
            }
        }
        ...
        private void OnRevertChangesToBase(object sender, Models.InstanceEventArgs e)
        {
            CreateObjectEditor();
        }
        ...
        protected virtual async void OnSelectedObjectsChanged(UnityObject[] unselectedObjects, UnityObject[] selectedObjects)
        {
            if (m_objectEditor != null && unselectedObjects != null && unselectedObjects.Length > 0)
            {
                if (Editor.IsDirty)
                {
                    await SaveObjects(unselectedObjects);
                    CreateObjectEditor();
                }
                else
                {
        ...
        private async Task SaveObjects(UnityObject[] objects)
        {
            var editor = IOC.Resolve<IRuntimeEditor>();
            ...
            var assetIDs = objects.Select(asset => editor.GetAssetID(asset)).Where(assetID => editor.CanEditAsset(assetID)).ToArray();
            foreach (var assetID in assetIDs)
            {
                await editor.SaveAssetAsync(assetID);
            }
            editor.IsBusy = false;
        }
        ...
        protected bool HasStaticObjects(UnityObject[] objects)
        {
            ...
            {
                var id = Editor.GetAssetID(obj);
                if (Editor.IsExternalAsset(id))
                {
                    return true;
                }
            }
