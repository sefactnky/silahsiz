...
namespace Battlehub.RTEditor.ViewModels
{
    ...
    public class AssetLibrarySelectViewModel : HierarchicalDataViewModel<AssetLibrarySelectViewModel.AssetLibrary>
    {
        ...
        {
            WindowManager.CreateWindow(RuntimeWindowType.ImportAssets.ToString());

            var assetLibraryImporter = IOC.Resolve<IAssetLibraryImportDialog>();
        ...
        private void OnAssetLibraryImporterClosed(object sender, IAssetLibraryImportDialog.CloseEventArgs args)
        {   
            if (args.Result != null)
            {
                var dialog = (IAssetLibraryImportDialog)sender;
