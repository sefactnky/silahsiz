...
namespace Battlehub.RTEditor.ViewModels
{
    ...
    public class ProjectTreeViewModel : HierarchicalDataViewModel<ProjectItem>, IProjectTreeModel
    {
        ...
        {
            RaiseParentChanged(oldParent, projectItem);
        }
        ...
        public void RemoveProjectItemsFromTree(ProjectItem[] projectItems)
        {
            for (int i = 0; i < projectItems.Length; ++i)
            {
                ...
                {
                    RaiseItemRemoved(projectItem.Parent, projectItem);
                }
            }
        ...
        protected virtual void CreatePrefab(ProjectItem dropTarget, IEnumerable<object> dragItems)
        {
            ...
            Editor.CreatePrefabAsync(TargetItem, dragObject, null);
        }
        ...
        protected virtual void AddItem(ProjectItem parentFolder, ProjectItem folder, string[] existingNames, bool select, bool expand)
        {
            ...
            RaiseItemAdded(parentFolder, folder);
            ...
            if (existingNames.Length > 0)
            {
                ...
                if (index > 0)
                {
                    RaiseNextSiblingChanged(folder, parentFolder.Children.Where(c => c.IsFolder).OrderBy(c => c.Name).ElementAt(index - 1));
                }
                else
                {
                    RaisePrevSiblingChanged(folder, parentFolder.Children.Where(c => c.IsFolder).OrderBy(c => c.Name).ElementAt(index + 1));
                }
            }
